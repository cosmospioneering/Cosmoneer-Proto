...
const uint16_t MOTORSTART = 230; // was 275...  this is the 'minimum' pulse length count (out of 4096)
...
const uint16_t MOTORIDLE = 250; // this is the 'minimum' pulse length count (out of 4096)
//const uint16_t MOTORMAX = 320; // 250this is the 'maximum' pulse length count (out of 4096)
const uint16_t MOTORMAX = 260; // 250this is the 'maximum' pulse length count (out of 4096)
// our servo # counter
...
int posLast = 315;    // variable to store the servo position 
int posCenter = 315;    // variable to store the servo position 
...
int posRight = 170;    // variable to store the servo position 
int posDelay = 10;
...
void intIMU(){
    ...
    //Setup the IMU for I2C pass-through
    mpu.setIntI2CMasterEnabled(false);
    mpu.setI2CBypassEnabled(true);
    mpu.setSleepEnabled(false);
    //i2c_writeReg(0x68, 0x37, 0x02);
    Serial.println(F("Passthrough setup compelte"));
...
void setServo(int sumPosition) {
  ...
  if (sumPosition >= posRight && sumPosition <= posLeft) {
    if (sumPosition < posLast)
    {
      posLast -= ((posLast - sumPosition)/4);
    }
    else
    {
      posLast += ((sumPosition - posLast)/4);
    }
  }
  delay(posDelay);
